...
type IRootState = ReturnType<GetStateFnType> // 通过ts工具ReturnType 获取函数返回值的类型,比如获取了counter的类型
type DispatchType = typeof store.dispatch // 获取dispatch的类型
// 2.把store重新包装TS类型限制，可参考redux官网
export const useAppSelect: TypedUseSelectorHook<IRootState> = useSelector // useSelector
